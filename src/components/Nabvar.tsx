    ...
    <header className="relative z-30 flex justify-between items-center px-6 md:px-0 md:pl-10 lg:pt-10">
      ...
      <ul
        className={`list-none bg-[#97979729] transition rounded-l-md backdrop-blur-xl fixed top-0 right-0
            h-screen w-64 z-0 pt-28 pl-8 md:relative md:h-24 md:w-fit md:px-12 md:pt-0 md:translate-x-0 md:flex md:justify-center md:items-center
            md:gap-x-12 lg:min-w-[50vw] ${
              !isOpen
                ? "translate-x-80"
                : isOpen
                ? "-translate-x-0"
                ...
                {isActive(item.href) && (
                  <motion.span
                    ...
                    className="absolute bottom-0 right-0
                              h-full w-1 md:w-full md:h-1 bg-white"
                  ></motion.span>
                )}
              </li>
            </Link>
